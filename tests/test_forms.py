...
from tests.testapp.forms import (
    ...
    TitleModelTomSelectWidget,
)
from tests.testapp.models import Artist, City, Country, Genre, Groupie


class TestTomSelectMixin:
    url = reverse("tom_select_widget")
    form = forms.AlbumTomSelectWidgetForm()
    multiple_form = forms.AlbumTomSelectMultipleWidgetForm()
    widget_cls = TomSelectWidget
    ...
    def test_initial_form_class(self):
        ...
        assert "django-tom-select" in widget.render("name", None)
    ...
    def test_selecting(self, db, live_server, driver):
        driver.get(live_server + self.url)
        with pytest.raises(NoSuchElementException):
            driver.find_element(By.CSS_SELECTOR, ".tom-select-results")
        elem = driver.find_element(By.CSS_SELECTOR, ".tom-select-selection")
        elem.click()
        results = driver.find_element(By.CSS_SELECTOR, ".tom-select-results")
        assert results.is_displayed() is True
        elem = results.find_element(By.CSS_SELECTOR, ".tom-select-results__option")
        elem.click()
    ...
    def test_theme_setting(self, settings):
        settings.TOM_SELECT_THEME = "classic"
        widget = self.widget_cls()
        assert 'data-theme="classic"' in widget.render("name", None)


class TestTomSelectMixinSettings:
    def test_default_media(self):
        sut = TomSelectWidget()
        result = sut.media.render()
        assert "django_tom_select/tom-select.complete.min.js" in result
        assert "django_tom_select/tom-select.css" in result
        assert "django_tom_select/django_tom_select.js" in result
        assert "django_tom_select/django_tom_select.css" in result

    def test_js_setting(self, settings):
        settings.TOM_SELECT_JS = "alternate.js"
        sut = TomSelectWidget()
        result = sut.media.render()
    ...
    def test_empty_js_setting(self, settings):
        settings.TOM_SELECT_JS = ""
        sut = TomSelectWidget()
        result = sut.media.render()
        assert "django_tom_select/django_tom_select.js" in result

    def test_css_setting(self, settings):
        settings.TOM_SELECT_CSS = "alternate.css"
        sut = TomSelectWidget()
        result = sut.media.render()
        assert "alternate.css" in result

    def test_empty_css_setting(self, settings):
        settings.TOM_SELECT_CSS = ""
        sut = TomSelectWidget()
        result = sut.media.render()
        assert "/tom-select.css" not in result

    def test_multiple_css_setting(self, settings):
        settings.TOM_SELECT_CSS = ["tom-select.css"]
        sut = TomSelectWidget()
        result = sut.media.render()
        assert "tom-select.css" in result


class TestHeavyTomSelectMixin(TestTomSelectMixin):
    url = reverse("heavy_tom_select_widget")
    form = forms.HeavyTomSelectWidgetForm(initial={"primary_genre": 1})
    widget_cls = HeavyTomSelectWidget
    ...
    def test_initial_form_class(self):
        ...
        assert "my-class" in widget.render("name", None)
        assert "django-tom-select" in widget.render("name", None)
        assert "django-tom-select-heavy" in widget.render("name", None), widget.render(
            "name", None
        )
    ...
    def test_many_selected_option(self, db, genres):
        field = HeavyTomSelectMultipleWidgetForm().fields["genres"]
    ...
    def test_multiple_widgets(self, db, live_server, driver):
        ...
        elem1, elem2 = driver.find_elements(By.CSS_SELECTOR, ".tom-select-selection")

        elem1.click()
        search1 = driver.find_element(By.CSS_SELECTOR, ".tom-select-search__field")
        search1.send_keys("fo")
        result1 = (
            WebDriverWait(driver, 60)
            .until(
                expected_conditions.presence_of_element_located(
                    (By.CSS_SELECTOR, ".tom-select-results li:first-child")
                )
            )
            .text
        )
        ...
        elem2.click()
        search2 = driver.find_element(By.CSS_SELECTOR, ".tom-select-search__field")
...
class TestModelTomSelectMixin(TestHeavyTomSelectMixin):
    form = forms.AlbumModelTomSelectWidgetForm(initial={"primary_genre": 1})
    multiple_form = forms.ArtistModelTomSelectMultipleWidgetForm()
    ...
    def test_tag_attrs_TomSelectWidget(self):
        widget = TomSelectWidget()
        output = widget.render("name", "value")
        assert 'data-minimum-input-length="0"' in output

    def test_custom_tag_attrs_TomSelectWidget(self):
        widget = TomSelectWidget(attrs={"data-minimum-input-length": "3"})
        output = widget.render("name", "value")
        assert 'data-minimum-input-length="3"' in output

    def test_tag_attrs_ModelTomSelectWidget(self):
        widget = ModelTomSelectWidget(
    ...
    def test_tag_attrs_ModelTomSelectTagWidget(self):
        widget = ModelTomSelectTagWidget(
            queryset=Genre.objects.all(), search_fields=["title__icontains"]
        )
        output = widget.render("name", "value")
        assert 'data-minimum-input-length="2"' in output

    def test_tag_attrs_HeavyTomSelectWidget(self):
        widget = HeavyTomSelectWidget(data_url="/foo/bar/")
        output = widget.render("name", "value")
        assert 'data-minimum-input-length="2"' in output

    def test_custom_tag_attrs_ModelTomSelectWidget(self):
        widget = ModelTomSelectWidget(
    ...
    def test_filter_queryset(self, genres):
        widget = TitleModelTomSelectWidget(queryset=Genre.objects.all())
        assert widget.filter_queryset(None, genres[0].title[:3]).exists()

        widget = TitleModelTomSelectWidget(
    ...
    def test_model_kwarg(self):
        widget = ModelTomSelectWidget(model=Genre, search_fields=["title__icontains"])
    ...
    def test_ajax_view_registration(self, client):
        ...
        url = reverse("django_tom_select:auto-json")
        genre = Genre.objects.last()
        response = client.get(
    ...
    def test_render(self):
        widget = ModelTomSelectWidget(queryset=Genre.objects.all())
        widget.render("name", "value")
...
class TestHeavyTomSelectTagWidget(TestHeavyTomSelectMixin):
...
class TestHeavyTomSelectMultipleWidget:
    url = reverse("heavy_tom_select_multiple_widget")
    form = forms.HeavyTomSelectMultipleWidgetForm()
    widget_cls = HeavyTomSelectMultipleWidget
    ...
    def test_widgets_selected_after_validation_error(self, db, live_server, driver):
        ...
        genres, fartists = driver.find_elements(
            By.CSS_SELECTOR, ".tom-select-selection--multiple"
        )
        genres.click()
        genres.send_keys("o")  # results are Zero One Two Four
        # select second element - One
        driver.find_element(By.CSS_SELECTOR, ".tom-select-results li:nth-child(2)").click()
        genres.submit()
        ...
        result_title = driver.find_element(
            By.CSS_SELECTOR, ".tom-select-selection--multiple li"
        ).get_attribute("title")
        assert result_title == "One"


class TestAddressChainedTomSelectWidget:
    url = reverse("model_chained_tom_select_widget")
    form = forms.AddressChainedTomSelectWidgetForm()
    ...
    def test_widgets_selected_after_validation_error(
        ...
        WebDriverWait(driver, 60).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".tom-select-selection--single")
            )
        )
        (
            country_container,
            city_container,
            city2_container,
        ) = driver.find_elements(By.CSS_SELECTOR, ".tom-select-selection--single")

        # clicking city tom-select lists all available cities
        city_container.click()
        WebDriverWait(driver, 60).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".tom-select-results li")
            )
        )
        city_options = driver.find_elements(By.CSS_SELECTOR, ".tom-select-results li")
        ...
        WebDriverWait(driver, 60).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".tom-select-results li:nth-child(2)")
            )
        )
        ...
        # clicking city tom-select lists reduced list of cities belonging to the country
        city_container.click()
        WebDriverWait(driver, 60).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".tom-select-results li")
        ...
        # clicking country tom-select lists reduced list to the only country available to the city
        country_container.click()
        WebDriverWait(driver, 60).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".tom-select-results li")
        ...
        country_options = driver.find_elements(By.CSS_SELECTOR, ".tom-select-results li")
    ...
    def test_dependent_fields_clear_after_change_parent(
        ...
        WebDriverWait(driver, 60).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".tom-select-results li:nth-child(3)")
            )
        )
        country_option = driver.find_element(
            By.CSS_SELECTOR, ".tom-select-results li:nth-child(3)"
...
@pytest.fixture(
    ...
    params=[
        (TomSelectWidget, {}),
        (HeavyTomSelectWidget, {"data_view": "heavy_data_1"}),
        (HeavyTomSelectMultipleWidget, {"data_view": "heavy_data_1"}),
        (ModelTomSelectWidget, {}),
        (ModelTomSelectTagWidget, {}),
    ],
    ids=lambda p: p[0],
